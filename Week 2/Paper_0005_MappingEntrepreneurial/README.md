...
The script reads `../paper_mappingentrepreneurialinclusion/mapping_entrepreneurial_inclusion.dta`
on first run to build `DATA.csv`, then proceeds with simulation. Structured logs
are written to `simulation_0005.log` (gitignored). Per-iteration regression outputs
are written to `regression_txt_outputs/` (gitignored).
