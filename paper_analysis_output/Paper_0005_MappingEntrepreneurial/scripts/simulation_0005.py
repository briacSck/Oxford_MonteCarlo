...
Estimator: pyfixest.feols(), CRV1 clustered on MSA_fe
Two-way FE: | state_name_fe + MSA_fe
"""
...
LOG_FILE = PAPER_DIR / "logs" / "simulation_0005.log"
...
class Config:
    ...
    REPORT_WORKBOOK = PAPER_DIR / "full_run" / "Stroube2025Report_0005.xlsx"
    SMOKE_WORKBOOK = PAPER_DIR / "smoke" / "SMOKE_Stroube2025Report_0005.xlsx"
    REGRESSION_TXT_DIR = PAPER_DIR / "regression_outputs"
    PROGRESS_LOG = PAPER_DIR / "logs" / "simulation_0005_progress.log"
...
def load_and_inspect_data(force_reload: bool = False) -> pd.DataFrame:
    ...
    try:
        import pyreadstat
        df, _ = pyreadstat.read_dta(str(SOURCE_DTA))
...
def _print_inspection_report(df: pd.DataFrame, source: str) -> None:
    ...
    for v in key_vars:
        if v in df.columns:
            s = df[v].describe()
            print(f"  OK  {v:<40} mean={s['mean']:9.4f}  std={s['std']:9.4f}  missing={df[v].isnull().sum()}")
        else:
            print(f"  ABSENT: {v}")
    print("=" * 72)
...
def _run_simulation_regression(df: pd.DataFrame):
    ...
    formula = f"{y} ~ {' + '.join(x)} | {fe}"
    df_clean = df[[y] + x + Config.FE_VARS].dropna()
    if len(df_clean) < 50:
        raise ValueError(f"Too few observations: {len(df_clean)}")
    fit = pf.feols(formula, data=df_clean, vcov={"CRV1": Config.CLUSTER_VAR})
    return fit
...
def run_baseline_regression(df: pd.DataFrame):
    ...
    fe = " + ".join(Config.FE_VARS)
    formula = f"{y} ~ {' + '.join(x)} | {fe}"
    df_reg = df[[y] + x + Config.FE_VARS].dropna()
    log.info(f"Regression sample N = {len(df_reg):,}")
    fit = pf.feols(formula, data=df_reg, vcov={"CRV1": Config.CLUSTER_VAR})
...
def _print_baseline_table(fit, n_obs: int) -> None:
    sep = "=" * 72
    print(sep)
    print(f"BASELINE REGRESSION -- Paper Table 3, Model 3_1  N={n_obs:,}")
    print(sep)
    tidy = fit.tidy()
    for var in [Config.FOCAL_IV] + Config.CONTROLS:
        if var in tidy.index:
            ...
            p = float(tidy.loc[var, "Pr(>|t|)"])
            sig = "***" if p < 0.001 else "**" if p < 0.01 else "*" if p < 0.05 else ""
            print(f"  {var:<43} {c:10.4f} {s:10.4f} {p:10.4f} {sig}")
    try:
...
def _extract_focal_coef(fit) -> dict:
    """Extract focal IV coefficient info from pyfixest result or pooled MI object."""
    if fit is None:
        return None
    try:
        if getattr(fit, "_is_pooled_mi", False):
            ...
            # pyfixest uses _N (not .nobs)
            nobs = int(getattr(fit, "_N", getattr(fit, "nobs", 0)))
        return {
...
def _pool_mi_results(fits: list, key_var_coefs: list = None):
    """Pool M fitted models using Rubin's Rules. Returns SimpleNamespace with MI diagnostics."""
    valid = [(f, c) for f, c in zip(fits, key_var_coefs or [None]*len(fits)) if f is not None]
    if not valid:
        return None
    M = len(valid)
    coefs = np.array([c["coef"] for _, c in valid if c is not None and pd.notna(c["coef"])])
    ses = np.array([c["se"] for _, c in valid if c is not None and pd.notna(c["se"])])
    nobs_list = [c["nobs"] for _, c in valid if c is not None]
    if len(coefs) == 0:
        return None
    M_v = len(coefs)
...
def write_excel_report(
    ...
    if hasattr(baseline_fit, "tidy"):
        tidy = baseline_fit.tidy().copy()
        tidy.index.name = "Variable"
        reg_df = tidy.reset_index()
    else:
        reg_df = pd.DataFrame({"Variable": ["pyfixest result not available"]})
    ...
    if coef_records:
        ...
        # RMSE of coef vs baseline focal coef
        ...
        # Summary by method × mechanism × pct
        if "coef_dev" in comp_df.columns:
            model_comp = comp_df.groupby(["method", "mechanism", "pct_str"]).agg(
                rmse=("coef_dev", lambda x: np.sqrt(np.mean(x**2))),
                avg_rel_se=("rel_se", "mean") if "rel_se" in comp_df.columns else ("coef_dev", "count"),
                n_iters=("coef", "count"),
            ).reset_index()
        else:
    ...
    # Average B-proportion per method across all scenarios
    if summary_rows:
...
def _compute_nmar_residual(df: pd.DataFrame, baseline_fit, key_vars: list) -> list:
    ...
    try:
        ...
        df_reg = df[[y] + x + fe_vars].dropna()
        # Get baseline residuals (predicted values from baseline)
        tidy = baseline_fit.tidy()
        intercept_estimate = 0.0  # FE absorbed; use predicted from pyfixest if available
        # pyfixest doesn't expose residuals directly; compute from .fitted_values if available
        try:
            fitted = baseline_fit.fitted_values()
            resids = df_reg[y].values - fitted.values
        except Exception:
            resids = np.zeros(len(df_reg))

        for kv in key_vars:
            # Simulate NMAR at various proportions
            for prop in [0.20]:
                ...
                # Align with df_reg index
                common = df_reg.index.intersection(miss_indicator.index)
                if len(common) < 20:
                    continue
                X_logit = pd.DataFrame({"resid": resids[:len(common)] if len(resids) >= len(common) else np.zeros(len(common))})
                y_logit = miss_indicator.loc[common].values
                try:
...
def run_simulation(df: pd.DataFrame, baseline_fit, mode: str) -> None:
        ...
        for prop in levels:
            ...
            for key_var in key_vars:
                ...
                for iteration in range(n_iters):
                    ...
                    # Build pipeline once per (key_var, iteration, missingness scenario)
                    pipeline = ImputationPipeline(
                    ...
                    for method in Config.METHODS:
                        ...
                        try:
                            ...
                            if method == "LD":
                                ...
                                for m_idx, mi_df in enumerate(mi_datasets):
                                    try:
                                        ...
                                        if sub_ci:
                                            per_m_coefs.append(sub_ci["coef"])
                            ...
                            if coef_info:
                                ...
                                # Imputed stats (variance and skewness of imputed column)
                                try:
                                ...
                                # Distribution comparison (imputed vs original)
                                try:
    ...
    # Post-hoc NMAR diagnostics (only for full run or if smoke)
...
# =============================================================================
def main():
    parser = argparse.ArgumentParser(description="Paper 0005 simulation")
    ...
    Config.PROGRESS_LOG = PAPER_DIR / "logs" / f"simulation_0005_progress_{args.mode}.log"
